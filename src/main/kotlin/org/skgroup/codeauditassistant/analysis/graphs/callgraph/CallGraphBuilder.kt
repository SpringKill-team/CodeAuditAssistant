...
import org.skgroup.codeauditassistant.analysis.di.DIProcessor
import org.skgroup.codeauditassistant.enums.RefMode
import org.skgroup.codeauditassistant.utils.GraphUtils
import org.skgroup.codeauditassistant.utils.PsiUtil.findConcreteImplementations
import java.util.*
...
class CallGraphBuilder : JavaRecursiveElementVisitor() {
    ...
    // 方法信息列表
    private val methodSigGraph = MethodSigGraph()
    ...
    // 用于表示是否构建调用路径的标志
    private var buildWithPath = System.getProperty("BUILD_WITH_PATH")?.toBoolean() ?: false
    ...
    override fun visitMethod(method: PsiMethod) {
        ...
        if (buildWithInfo) methodSigGraph.nodes.add(GraphUtils.getMethodSigNode(method))
...
//        val clazz: PsiClass? = constructor.containingClass
    ...
    fun getMethodSigGraph(): MethodSigGraph {
        return methodSigGraph
    }
