...
import org.skgroup.codeauditassistant.analysis.ast.nodes.BaseAstNode
...
import org.skgroup.codeauditassistant.enums.AnalysisScope
import org.skgroup.codeauditassistant.i18n.CAMessage
import org.skgroup.codeauditassistant.ui.renderer.MethodListRenderer
import org.skgroup.codeauditassistant.ui.renderer.ResultTreeRenderer
...
class CallGraphUIComponents(val project: Project) {
    // 所有UI组件声明
    val runAnalysisButton = JButton(CAMessage.message("callgraph.generate.button"))
    ...
    val searchResultRootNode = DefaultMutableTreeNode(CAMessage.message("callgraph.search.results"))
    ...
    val searchButton = JButton(CAMessage.message("callgraph.search.button"))

    val findMethodButton = JButton(CAMessage.message("callgraph.find.method.button"))
    val containInfo = JBCheckBox(CAMessage.message("callgraph.contains.info")).apply {
        addActionListener {
    ...
    val containPath = JBCheckBox(CAMessage.message("callgraph.contains.path")).apply {
        addActionListener {
            val enabled = isSelected
    ...
    val classNameLabel = JBLabel(CAMessage.message("callgraph.class.name"))
    val classNameField = JBTextField(20)

    val accessModifierLabel = JBLabel(CAMessage.message("callgraph.access.modifier"))
    val accessModifierField = JBTextField(10)

    val methodModifierLabel = JBLabel(CAMessage.message("callgraph.method.modifier"))
    val methodModifierField = JBTextField(10)

    val methodNameLabel = JBLabel(CAMessage.message("callgraph.method.name"))
    val methodNameField = JBTextField(15)

    val paramCountLabel = JBLabel(CAMessage.message("callgraph.param.count"))
    val paramCountField = JBTextField(5)

    val paramTypeLabel = JBLabel(CAMessage.message("callgraph.param.type"))
    val paramTypeField = JBTextField(15)

    val paramNameLabel = JBLabel(CAMessage.message("callgraph.param.name"))
    val paramNameField = JBTextField(15)

    val varargsLabel = JBLabel(CAMessage.message("callgraph.varargs"))
    val varargsField = JBTextField(5)

    val throwsClauseLabel = JBLabel(CAMessage.message("callgraph.throws.clause"))
    val throwsClauseField = JBTextField(15)

    val returnTypeLabel = JBLabel(CAMessage.message("callgraph.return.type"))
    val returnTypeField = JBTextField(15)

    val annotationsLabel = JBLabel(CAMessage.message("callgraph.annotations"))
    val annotationsField = JBTextField(20)

    val systemPlatformLabel =
        JBLabel(CAMessage.message("status.system.platform", System.getProperty("os.name")), LEFT).apply {
    ...
    val graphStatusLabel = JBLabel(CAMessage.message("status.callgraph.not.ready"), IconUtil.graphNotReadyIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }

    val methodLabel = JBLabel(CAMessage.message("status.method.count", 0), IconUtil.methodNotReadyIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }

    val memoryLabel = JBLabel(CAMessage.message("status.memory.usage", 0), IconUtil.memoryLowIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }

    val buildInfoLabel = JBLabel(CAMessage.message("status.build.info.none"), IconUtil.normalIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }

    val searchInfoLabel = JBLabel(CAMessage.message("status.search.info.none"), IconUtil.normalIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }

    val errorInfoLabel = JBLabel(CAMessage.message("status.error.none"), IconUtil.errorIcon, LEFT).apply {
        border = JBUI.Borders.empty(5)
    }
    ...
    val toggleButton = JButton(CAMessage.message("button.toggle.panel"))
    ...
    private fun createSearchPanel(): JBPanel<JBPanel<*>> {
        ...
        c.gridy = 0
        c.weightx = 0.0
        panel.add(JBLabel(CAMessage.message("search.label.root")), c)

        c.gridx = 1
        panel.add(sourceField, c)

        c.gridx = 2
        panel.add(JBLabel(CAMessage.message("search.label.sink")), c)
    ...
    private fun createCenterPanel(): JComponent {
        ...
        // 创建各个功能面板
        val rootPanel = createTitledPanel(CAMessage.message("panel.title.root.methods"), JBScrollPane(rootList))
        val infoPanel = createTitledPanel(CAMessage.message("panel.title.info"), JBScrollPane(statusPanel))
        val searchResultPanel = createTitledPanel(CAMessage.message("panel.title.search.results"), JBScrollPane(searchResultTree))
    ...
    fun addIssueProblemsTabToProblemsView() {
        ...
        val content = ContentFactory.getInstance().createContent(issueTab.getComponent(), CAMessage.message("tab.sink.finder"), false)
    ...
    fun updateInfoPanel(callGraph: CallGraph) {
        ...
        graphStatusLabel.text = CAMessage.message("status.callgraph.ready")
        graphStatusLabel.icon = IconUtil.graphReadyIcon
        methodLabel.text = CAMessage.message("status.method.count", callGraph.nodes.size)
        methodLabel.icon = IconUtil.methodReadyIcon
        memoryLabel.icon = IconUtil.graphReadyIcon
        memoryLabel.text = CAMessage.message("status.memory.usage", usedMemory)
        when (usedMemory) {
    ...
    fun addBuildInfo(buildInfo: String) {
        buildInfoLabel.text = CAMessage.message("status.build.info", buildInfo)
    }

    fun addSearchInfo(searchInfo: String) {
        searchInfoLabel.text = CAMessage.message("status.search.info", searchInfo)
    }

    fun addErrorInfo(errorInfo: String) {
        errorInfoLabel.text = CAMessage.message("status.error", errorInfo)
    }
